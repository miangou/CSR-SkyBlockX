...
namespace sbx
{
    class sbbx
    {
...

        {
            ...
            api.setCommandDescribe("is money?", "查询金币");
            api.setCommandDescribe("is help", "is帮助界面");
            ...
            if (a.cmd.StartsWith("/is"))
            {
                    ...
                    if (iscommand == "money?")
                    {
                        var b = api.getscoreboard(uuid[a.playername], "money");
                        api.runcmd("tellraw \"" + a.playername + "\" {\"rawtext\":[{\"text\":\"§3您的金币为:"+b+"！\"}]}");
                        return false;
                    }
                    if (iscommand == "clear")
                    {
                    ...
                    if (iscommand == "help")
                    {
                        api.runcmd("tellraw \"" + a.playername + "\" {\"rawtext\":[{\"text\":\"§2——————————————————————\"}]}");
                        api.runcmd("tellraw \"" + a.playername + "\" {\"rawtext\":[{\"text\":\"§3输入/is creative 来创建岛屿\"}]}");
                        api.runcmd("tellraw \"" + a.playername + "\" {\"rawtext\":[{\"text\":\"§3输入/is go 来回到传送点\"}]}");
                        api.runcmd("tellraw \"" + a.playername + "\" {\"rawtext\":[{\"text\":\"§3输入/is goreset 来重置传送点\"}]}");
                        api.runcmd("tellraw \"" + a.playername + "\" {\"rawtext\":[{\"text\":\"§3输入/is sign 来签到\"}]}");
                        api.runcmd("tellraw \"" + a.playername + "\" {\"rawtext\":[{\"text\":\"§3输入/is money? 来查询金币\"}]}");
                        api.runcmd("tellraw \"" + a.playername + "\" {\"rawtext\":[{\"text\":\"§2——————————————————————\"}]}");
                        return false;
                    }
                    if (iscommand == null)
                    {
                        api.runcmd("tellraw \"" + a.playername + "\" {\"rawtext\":[{\"text\":\"§3请选择一个选项！\"}]}");
                        return false;
                    }                    
                    if (iscommand == "sign")
                    {
